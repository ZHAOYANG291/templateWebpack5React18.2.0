...
let webpackBaseConfig={
    // 默认打包成web平台的
    target: 'web',
    // 环境 development(开发) 和 production(生产) 环境
    mode: 'development',
    devServer: {
        ...
        port: 9003, //开启端口为9000   也可以直接在命令行中指定端口 webpack-dev-server --port 9000
    ...
    module: {
        rules: [
            //--------- 样式相关的loader     ---------start
            //npm i style-loader css-loader less less-loader sass sass-loader --save-dev
            {
                ...
                use: [{
                    ...
                    options: {
                        presets: [
                            ...
                            //将jsx的语法转化为浏览器能识别的js
                            ["@babel/preset-react", {
                                "runtime": "automatic" //在使用自动引入时，你不再需要在每个文件中引入 import React from 'react'，因为 Babel 会自动注入。这减小了生成的代码体积。
                            }],
                            //将ts转化为js、tsx转化为jsx
                            [
            ...
            },
            {
                ...
                generator: {
                    filename: (pathData) => {
                        ...
                        const basePath = isVideo ? 'assets/videos/' : 'assets/audios/';
            ...
            //--------- 添加各个类型文件的规则，排除 .woff 和 .ttf 文件 ---------start
            {
            ...
            //--------- 添加各个类型文件的规则，排除 .woff 和 .ttf 文件 ---------end
        ]
    },
